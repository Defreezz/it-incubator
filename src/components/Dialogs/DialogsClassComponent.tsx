...
import {AddMessageForm, AddMessageFormRedux} from "./AddMessageForm";
...
export class DialogsClassComponent extends React.Component<DialogsComponentType> {
    addNewMessage (message:AddMessageForm ){
        this.props.sendMessage(message.newMessageBody)
    }
    render() {
        const dialogsElement = this.props.dialogsPage.dialogs.map(d =>
            <DialogsItem key={d.id} id={d.id} name={d.name}/>);
        const messagesElement = this.props.dialogsPage.messages.map(m =>
